...
The structured article manifest lists every article with its title, authors, miniabstract, DOI, article type, symposium, and Blob URLs for the full-text XHTML, abstracts, figures, and data files. It is approximately 16 KB and fits within a single response.
...
If you are building a pipeline that ingests JEP articles, use the manifest as your structured index. Each article entry contains fully-qualified URLs for all resources. 
